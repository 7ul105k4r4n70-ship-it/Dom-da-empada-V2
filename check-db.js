...
async function checkData() {
  ...
  console.log('\n2️⃣ ORDER_ITEMS (preços salvos nos pedidos):');
  ...
  // 3. Ver tabela prices (se existe)
  console.log('\n3️⃣ TABELA PRICES (se existir):');
  const { data: prices, error: pricesError } = await supabase
    .from('prices')
    .select('product_id, cost_price, sale_price, valid_from')
    .limit(5);
  
  if (pricesError) {
    console.log('Tabela prices não existe ou está vazia:', pricesError.message);
  } else {
    console.log(`Total de registros em prices: ${prices?.length || 0}`);
    prices?.forEach(p => {
      console.log(`  - Product ID: ${p.product_id} | Custo: ${p.cost_price} | Venda: ${p.sale_price}`);
    });
  }
